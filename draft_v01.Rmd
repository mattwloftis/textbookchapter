...
As a concept, *n*=all implies that big data can be taken at face value. Its patterns reveal the complete picture of human behavior. Unfortunately, the selection problem is still with us. It arises whenever observations enter your data for reasons that systematically relate to the outcome variable. @AP08 illustrate the problem with an example from the United States' National Health Interview Survey, in which individuals are asked to rate their health on a scale. Perhaps unsurprisingly, they find that individuals in hospital consistently rate their health as worse than those out of hospital. Then they pose an interesting question: should we conclude that hospitals make people sicker? The answer is, of course, no. We know that because we already understand that individuals in hospital *selected* into being there precisely because they were sick. Failing to recognize that would result in sampling bias.
